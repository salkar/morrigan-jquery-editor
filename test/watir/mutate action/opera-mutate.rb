...
describe "Opera mutate actions" do
	...
	it "P should be mutated to H1 when selected range is on P (several p with scroll)" do
		...
		scroll_script = "$('iframe').contents().scrollTop(10000); return true;"
	...
	it "Selection should be restored after mutate action" do
		...
		selection_script << "rng.setEnd(b.firstChild,3);"
		selection_script << "selection.removeAllRanges();"
		selection_script << "selection.addRange(rng); return true;"
		@b.execute_script selection_script
		...
		@b.body.click
		@b.execute_script("return $('iframe')[0].contentWindow.getSelection().getRangeAt(0).toString();").should == "ragraph 21paragraph 22par"
	end
